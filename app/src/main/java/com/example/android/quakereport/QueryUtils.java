...
public final class QueryUtils {
    ...
    public static ArrayList<Earthquake> extractEarthquakes() {
        ...
        try {
            ...
            JSONArray earthquakesJSONArray = jsonRootObject.optJSONArray("features");

            for (int i = 0; i < earthquakesJSONArray.length(); i++) {
                //Get earthquake from JSONArray
                JSONObject currentEarthquake = earthquakesJSONArray.getJSONObject(i);

                //Get properties of current earthquake
                JSONObject currentEarthquakeProperties = currentEarthquake.getJSONObject("properties");

                //Get magnitude
                double currentEarthquakeMagnitude = currentEarthquakeProperties.getDouble("mag");

                //Get location
                String currentEarthquakeLocation = currentEarthquakeProperties.getString("place");

                //Get time of occurence
                long currentEarthquakeTime = currentEarthquakeProperties.getLong("time");

                //Get url
                String currentEarthquakeURL = currentEarthquakeProperties.getString("url");

                //Create Earthquake object and add to Array
                earthquakes.add(new Earthquake(currentEarthquakeMagnitude, currentEarthquakeLocation, currentEarthquakeTime,currentEarthquakeURL));
